...
const FreeFall: React.FC<{ title: string }> = ({ title }) => {
  ...
  const handleReset = () => {
    if (ball1Ref.current && ball2Ref.current && engineRef.current) {
      ...
      // オブジェクトを一旦削除
      Matter.Composite.remove(engine.world, [ball1Ref.current, ball2Ref.current]);
    ...
    >
      <div className="flex flex-col gap-y-6">
        ...
        <div className="space-y-3">
          <div>
            <h3 className="text-base font-bold text-white mb-1">シミュレーション設定</h3>
